...
private[projection] class JdbcOffsetStoreStatements(system: ActorSystem[_], settings: JdbcSettingsBase, clock: Clock) {

  private val logger = LoggerFactory.getLogger(this.getClass)
  private val verboseLogging = logger.isDebugEnabled() && settings.verboseLoggingEnabled

  def this(system: ActorSystem[_], settings: JdbcSettingsBase) =
    this(system, settings, Clock.systemUTC())

  private val offsetSerialization = new OffsetSerialization(system)
  import offsetSerialization.fromStorageRepresentation
  import offsetSerialization.toStorageRepresentation

  def clearOffset(conn: Connection, projectionId: ProjectionId): Done = {
    logger.debug(
  ...
  def readOffset[Offset](conn: Connection, projectionId: ProjectionId): Option[Offset] = {
    if (verboseLogging)
  ...
  def readManagementState(conn: Connection, projectionId: ProjectionId): Option[ManagementState] = {
    if (verboseLogging)
      logger.debug(
  ...
  def savePaused(conn: Connection, projectionId: ProjectionId, paused: Boolean): Done = {
    if (verboseLogging)
      logger.debugN(
