  ...
  public static void main(String[] args) throws Exception {
    ...
    ActorSystem.create(Behaviors.setup(context -> {
      ...
      SourceProvider<Offset, EventEnvelope<ShoppingCartEvents.Event>> sourceProvider =
        EventSourcedProvider.eventsByTag(system, CassandraReadJournal.Identifier(), ShoppingCartTags.Single);
      //#guideSourceProviderSetup

      //#guideProjectionSetup
      CassandraSession session = CassandraSessionRegistry.get(system).sessionFor("akka.projection.cassandra.session-config");
      ItemPopularityProjectionRepositoryImpl repo = new ItemPopularityProjectionRepositoryImpl(session);
      AtLeastOnceProjection<Offset, EventEnvelope<ShoppingCartEvents.Event>> projection = CassandraProjection.atLeastOnce(
        ProjectionId.of("shopping-carts", ShoppingCartTags.Single),
        sourceProvider,
        () -> new ItemPopularityProjectionHandler(ShoppingCartTags.Single, system, repo));
