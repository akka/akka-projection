#!/usr/bin/env bash
#
# release-prep.sh [--commit-and-pr] <akka-projection-version>
#
# Prepares the repository for a release by:
#  - Updating the LICENSE file:
#      * "Licensed Work: Akka Projection X.Y.Z" -> the given version
#      * "(c) YYYY Lightbend Inc."               -> the current year
#      * "Change Date: YYYY-MM-DD"               -> today + 3 years
#  - Bumping the akka-projection version in every sample build file:
#      * samples/**/build.sbt    (sys.props.getOrElse("akka-projection.version", "..."))
#      * samples/**/pom.xml      (<akka-projection.version>...</akka-projection.version>)
#
...
usage() {
  echo "Usage: $0 [--commit-and-pr] <akka-projection-version>" >&2
  echo "Example: $0 1.6.23" >&2
  echo "         $0 --commit-and-pr 1.6.23" >&2
}
...
AKKA_PROJECTION_VERSION="${POSITIONAL[0]}"
COMMIT_MSG="chore: License change date and sample bump for $AKKA_PROJECTION_VERSION"
...
if [ "$COMMIT_AND_PR" = true ]; then
  ...
  NEW_BRANCH="wip-release-prep-$AKKA_PROJECTION_VERSION"
...
echo "Preparing release of Akka Projection $AKKA_PROJECTION_VERSION"
...
"${SED_INPLACE[@]}" -E \
  -e "s/^(Licensed Work:[[:space:]]+Akka Projection )[^[:space:]]+/\\1$AKKA_PROJECTION_VERSION/" \
  -e "s/(The Licensed Work is \\(c\\) )[0-9]{4}( Lightbend Inc\\.)/\\1$CURRENT_YEAR\\2/" \
  -e "s/^(Change Date:[[:space:]]+)[0-9-]+/\\1$CHANGE_DATE/" \
  LICENSE

echo "Bumping samples to $AKKA_PROJECTION_VERSION..."
# build.sbt: the sys.props.getOrElse default is on its own (continuation) line.
find samples -name build.sbt -exec "${SED_INPLACE[@]}" -E \
  -e "s/sys\\.props\\.getOrElse\\(\"akka-projection\\.version\", \"[^\"]*\"\\)/sys.props.getOrElse(\"akka-projection.version\", \"$AKKA_PROJECTION_VERSION\")/" \
  {} +

# pom.xml: the <akka-projection.version> property.
find samples -name pom.xml -exec "${SED_INPLACE[@]}" -E \
  -e "s|<akka-projection\\.version>[^<]*</akka-projection\\.version>|<akka-projection.version>$AKKA_PROJECTION_VERSION</akka-projection.version>|" \
  {} +
