...
// TODO: copied from Alpakka Kafka PR, should we integrate this directly into projections or keep it upstream?
// https://github.com/akka/alpakka-kafka/pull/1090
object Processor {
  ...
  def atLeastOnceSource[K, V](
      ...
      process: ConsumerRecord[K, V] => Future[_]): Source[ConsumerMessage.CommittableOffsetBatch, Consumer.Control] = {
    Consumer
      .committableSource(consumerSettings, subscription)
      .mapAsync(1) { message =>
        process(message.record).map(_ => message.committableOffset)(ExecutionContexts.parasitic)
      }
  ...
  def atLeastOnceConsumeAndProduceSource[CK, CV, PK, PV](
      ...
      committerSettings: CommitterSettings)(process: ConsumerRecord[CK, CV] => Future[Envelope[PK, PV, NotUsed]])
      : Source[ConsumerMessage.CommittableOffsetBatch, Consumer.Control] =
    Consumer
      .committableSource(consumerSettings, subscription)
      .mapAsync(1) { message =>
        process(message.record).map { envelope => envelope.withPassThrough(message.committableOffset) }(
