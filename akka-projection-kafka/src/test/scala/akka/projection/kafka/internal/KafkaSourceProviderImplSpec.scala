...
object KafkaSourceProviderImplSpec {
  ...
  class TestProjection(
    ...
    override private[projection] def mappedSource()(implicit systemProvider: ClassicActorSystemProvider) = {
      ...
      Source
        ...
        .mapConcat { env =>
          ...
          sourceProvider.verifyOffset(offset) match {
            ...
            case SkipOffset(reason) => Nil
          }
        }
        .map(_ => Done)
    }

    override private[projection] def run()(implicit systemProvider: ClassicActorSystemProvider) = ???
  }
}
