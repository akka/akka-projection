...
import central.drones.{Drone, proto}
...
class DroneOverviewServiceImpl(system: ActorSystem[_]) extends DroneOverviewService {
  ...
  override def getCoarseDroneLocations(in: proto.CoarseDroneLocationsRequest): Future[proto.CoarseDroneLocationsResponse] = {
    ...
    R2dbcSession.withSession(system) { session =>
      session.select(session.createStatement(findByLocationSql).bind(0, in.location)) { row =>
        ...
        val state =
          serialization.deserialize(payload, serializerId, serializerManifest)
          .get
        ...
        if (locations.isEmpty) throw new GrpcServiceException(Status.NOT_FOUND)
        else proto.CoarseDroneLocationsResponse(locations.map(location => proto.CoarseDroneLocations(location.latitude, location.longitude)))
      }
    }
  }
}
