...
public class DeliveriesQueueServiceImpl implements DeliveriesQueueService {
    ...
    public DeliveriesQueueServiceImpl(ActorSystem<?> system, Settings settings, ActorRef<DeliveriesQueue.Command> deliveriesQueue) {
        this.system = system;
    ...
    public CompletionStage<GetCurrentQueueResponse> getCurrentQueue(GetCurrentQueueRequest in) {
        ...
        private GetCurrentQueueResponse toProto(DeliveriesQueue.State state) {
            return GetCurrentQueueResponse.newBuilder()
                    .addAllWaitingDeliveries(state.waitingDeliveries.stream().map(this::waitingToProto).collect(Collectors.toList()))
                    .addAllDeliveriesInProgress(state.deliveriesInProgress.stream().map(this::deliveryInProgressToProto).collect(Collectors.toList()))
                    .build();
        }
        ...
        private DeliveryInProgress deliveryInProgressToProto(DeliveriesQueue.DeliveryInProgress inProgress) {
