...
public final class DeliveriesQueue extends DurableStateBehavior<DeliveriesQueue.Command, DeliveriesQueue.State> {
    ...
    public static final class RequestDelivery implements Command {
        ...
        public RequestDelivery(String droneId, Coordinates droneCoordinates, ActorRef<StatusReply<WaitingDelivery>> replyTo) {
    ...
    public static final class State implements CborSerializable {
        ...
        public State(List<WaitingDelivery> waitingDeliveries, List<DeliveryInProgress> deliveriesInProgress) {
    ...
    private Effect<State> onAddDelivery(State state, AddDelivery command) {
        ...
        if (state.waitingDeliveries.contains(command.delivery) || state.deliveriesInProgress.stream().anyMatch(deliveryInProgress ->
            ...
            return Effect()
                    .persist(state)
                    .thenReply(command.replyTo, updatedState -> done());
        }
    }
    ...
    private Effect<State> onRequestDelivery(State state, RequestDelivery command) {
        if (state.waitingDeliveries.isEmpty()) {
            ...
            var closestPickupForDrone = state.waitingDeliveries.stream().min(Comparator.comparingLong(delivery ->
                            command.droneCoordinates.distanceTo(delivery.from)))
                    .get();
            context.getLog().info(
            ...
            state.deliveriesInProgress.add(new DeliveryInProgress(closestPickupForDrone.deliveryId, command.droneId, Instant.now()));
            return Effect().persist(state)
    ...
    private Effect<State> onCompleteDelivery(State state, CompleteDelivery command) {
        var maybeExisting = state.deliveriesInProgress.stream().filter(delivery -> delivery.deliveryId.equals(command.deliveryId)).findFirst();
        if (maybeExisting.isEmpty()) {
            return Effect().reply(command.replyTo, StatusReply.error("Unknown delivery id: " + command.deliveryId));
        } else {
