...
public class DeliveryEvents {
    ...
    public static Behavior<ProjectionBehavior.Command> projectionBehavior(
        ...
        var sliceRanges =
                Persistence.get(system).getSliceRanges(1);
        var sliceRange = sliceRanges.get(0);
        var projectionKey = eventsBySlicesQuery.streamId() + "-" + sliceRange.first() + "-" + sliceRange.second();

        var projectionId = ProjectionId.of(projectionName, projectionKey);

        var sourceProvider = EventSourcedProvider
                .<central.deliveries.proto.DeliveryRegistered>eventsBySlices(
                system,
                eventsBySlicesQuery,
        ...
        var handler = Handler.fromFunction((EventEnvelope<DeliveryRegistered> envelope) ->
                ask(queueActor,
        ...
        return ProjectionBehavior.create(
                R2dbcProjection
                        .atLeastOnceAsync(projectionId, Optional.empty(), sourceProvider, () -> handler, system));
