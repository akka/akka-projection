...
private[projection] sealed trait HandlerStrategy[Envelope] {
  ...
  def actorHandlerInit[M]: Option[ActorHandlerInit[M]]
}
...
private[projection] final case class SingleHandlerStrategy[Envelope](handler: Handler[Envelope])
  ...
  override def actorHandlerInit[M]: Option[ActorHandlerInit[M]] = handler match {
    case init: ActorHandlerInit[M] @unchecked => Some(init)
    case _                                    => None
  }
}
...
private[projection] final case class FlowHandlerStrategy[Envelope](
  ...
  override def actorHandlerInit[M]: Option[ActorHandlerInit[M]] = None
}
