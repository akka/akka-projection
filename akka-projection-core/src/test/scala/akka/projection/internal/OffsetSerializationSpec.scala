...
class OffsetSerializationSpec extends TestSuite with Matchers with AnyWordSpecLike {
  ...
  "OffsetSerialization of MergeableOffsets" must {
    "convert offsets of type MergeableOffset" in {
      ...
      val mergeableOffset = MergeableOffsets.Offset(Map(surrogateProjectionKey -> 1L))
      val actualRep = toStorageRepresentation[MergeableOffsets.Offset[Long]](id, mergeableOffset)
      ...
      fromStorageRepresentation[MergeableOffsets.Offset[Long], Long](storageRepresentation) shouldBe mergeableOffset
    }
    ...
    "merge rows into one MergeableOffset" in {
      ...
      val mergeableOffset = MergeableOffsets
        .Offset(Map(surrogateProjectionKey1 -> 1L, surrogateProjectionKey2 -> 2L))
      val storageRepresentation = MultipleOffsets(
        Seq(
